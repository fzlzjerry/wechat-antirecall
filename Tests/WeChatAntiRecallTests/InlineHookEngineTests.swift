...
final class InlineHookEngineTests: XCTestCase {
    ...
    func testEncoderMatchesRecorded269339StaticPatch() throws {
        var bytes = [UInt8](repeating: 0, count: 12)
        XCTAssertEqual(wechat_antirecall_encode_entry_stub(0x462e240, 0x9a9ff00, &bytes), 1)
        let hex = bytes.map { String(format: "%02X", $0) }.joined()
        XCTAssertEqual(hex, "90A302B0108247F900021FD6")
        XCTAssertEqual(wechat_antirecall_decode_entry_stub_slot(&bytes, 0x462e240), 0x9a9ff00)
    }
