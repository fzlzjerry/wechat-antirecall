...
短语最长 120 个字符，不能包含换行。`{from}` 会在运行时替换成发送者备注或昵称，`{time}` 会替换成撤回时间（`HH:mm`）。runtime 会优先使用撤回 XML 里的时间字段；如果没有可用字段，则固定为首次拦截到同一撤回事件时的本地时间。已经渲染过的自定义提示会原样保留，不会因为重新 patch 或重新打开会话而刷新时间。未配置时默认显示 `已拦截一条撤回消息`。
