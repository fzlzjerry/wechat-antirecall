...
- **无限多开（旧方案）**：加 `--multi-instance`。当前仅构建号 `268575` / `268599` 支持；部分 4.1.9 构建上可能触发重复权限弹窗，不再推荐作为主路径。
  重复权限弹窗解决：设置 - 隐私与安全性 - 完全磁盘访问权限(或者重复弹窗的对应权限)： 选择`微信`,点列表底部 `-` 删除，再点列表底部 `+` 选择 `微信`， 添加后会弹出提示窗，选择 退出并重新打开 生效
...
- 当前只保留每个 entry 都有 `expected` 校验的配置；新增或恢复构建号时必须先补齐原始字节校验。
