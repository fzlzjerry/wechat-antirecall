...
| 269136 | `0x48a0140` | `0x93a3f00` | 微信 4.1.11 App Store 构建；不含 `XAppUpdateManager` |
...
- **269136**：4.1.11 App Store 构建，`parseRevokeXML` 的几何特征仍在整个 arm64 切片里唯一命中，入口重定位到 `0x48a0140`；`revoke` `0x48a03b0`、`revoke-tip` `0x48a0b44` 和入口原始字节均逐地址核对。字段访问仍为 `0x168`/`0x170`。SLOT 取 `__common` 结束地址 `0x93a3098` 之后、`__DATA` 结束地址 `0x93a4000` 之前的零填充 `0x93a3f00`，入口桩编码回环解码到同一地址。该二进制不含 `XAppUpdateManager` 及其更新 selector，因此不提供 `update` 目标。
