...
| 269339 | `0x462e240` | `0x9a9ff00` | 微信 4.1.12 热修（产品版本 4.1.12.27）；269332+ 几何特征仍唯一命中，字段偏移仍 `0x198`/`0x1A0` |
...
- **269339**：微信 4.1.12 热修（`CFBundleVersion` 269339，`WeChatBundleVersion` 4.1.12.27，当前 macOS 安装版）。269332+ 的函数几何特征在整个 arm64 切片中唯一命中：入口 `0x462e240`，`entry+0x270` 的 `cbz w0,+0x208` 在 `0x462e4b0`，`entry+0xA10` 的 `str x0,[x19,#0x198]` 在 `0x462ec50`，函数体内只有一处该 store 和四处 `ldr x0,[x19,#0x1A0]`。字段偏移从本构建指令重新解码为 `0x198`/`0x1A0`。SLOT `0x9a9ff00` 位于 `__common` 结束地址 `0x9a9ea68` 与 `__DATA` 结束地址 `0x9aa0000` 之间的零填充，入口桩（`90A302B0108247F900021FD6`）回环解码到同一地址。屏蔽更新 8 处从本构建 `XAppUpdateManager` 的相对方法表按 selector 名重新解析：`startUpdater` `0x26e4c0`、`checkForUpdates:` `0x2706ec`、`startBackgroundUpdatesCheck:` `0x2709bc`、`enableAutoUpdate:` `0x270ddc`，访问器对 `0x27b1c0`/`0x27b1c8`/`0x27b1d0`/`0x27b1d8`；8 处原始字节逐点核对，访问器字段仍为 `0x18`/`0x19`。真实二进制的 silent / runtime-tip / block-update 三种 `install --dry-run` 均通过。
