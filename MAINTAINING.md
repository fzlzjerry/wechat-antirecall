...
| `runtime-tip` | 内联 hook 的入口改写（仅内联构建；269340 含两处） | `--runtime-tip` |
...
- 安装时把目标函数入口的 3 条指令改成 `adrp x16,SLOT; ldr x16,[x16]; br x16`（patches.json 的 `runtime-tip` 目标；269340 同时改写撤回解析器和 Message 终结器）。`SLOT` 落在 `wechat.dylib` `__DATA` 段尾的零填充空隙里。
...
| 269341 | `0x462e60c` + `0x45ce5b0` | `0x9a9ff00` + `0x9a9ff08` | 微信 4.1.12 热修（当前 macOS 安装版）。**整片 arm64 切片被重建**（非重定位），`parseRevokeXML` 与 Message 终结器均重定位，靠对 269340 参考二进制的 masked-shape diff 重新定位；入口与终结器都落在 269340 同一 4KB 页，故入口桩逐字节不变，SLOT 不变，字段偏移仍 `0x198`/`0x1A0`，屏蔽更新 8 处地址与字节**完全等同 269340**（`XAppUpdateManager` 区域未被重定位） |
...
IDA Pro 9.4 对 269340 arm64 切片的聚焦分析确认：`0x462e200` 虽带有 `TryParseMessageXML` 日志且 ABI 确实只有三个参数，但其包装函数 `0x462dfec` 位于类型扩展 vtable，类型列表函数 `0x462df44` 只返回 `71/72`。因此该入口不是所有接收消息的公共路径；输出对象 `+0x198` 也是撤回 XML 解析后写入的目标 `newmsgid`，不能当作普通消息的 server ID。

真正的接收路径使用第二个内联 hook：`sub_45CE1A4`（入口 `0x45ce1a4`）是通用 Message 终结器。网络消息构造函数 `sub_45CCE50` 在调用它之前已经填好：
...
269340 的 `runtime-tip` 因此包含两个静态入口改写：撤回解析器写入 `SLOT 0x9a9ff00`，Message 终结器写入相邻的 `SLOT 0x9a9ff08`。缓存只存在于当前微信进程，最多 512 条；文本预览最多 240 个 UTF-8 字节。冷启动前收到、已淘汰或 server ID 为 0 的消息仍会 miss，此时 `replaceContentPlaceholder` 会连同分隔符一起剥掉。其他已支持构建尚未反向确认通用 Message 字段布局，因此仍按冷缓存处理 `{content}`。
...
- **269340**：微信 4.1.12 热修（`CFBundleVersion` 269340，当前 macOS 安装版）。在 IDA Pro 9.4 中对 arm64 切片做聚焦分析：包装函数 `0x462dfec` 调用的核心函数 `0x462e200` 带有 `TryParseMessageXML` 日志字符串，调用点明确只传 `x0` 输出对象、`x1` 原始内容 `std::string *`、`x2` 标志指针三个参数；同时 vtable `0x9479db0` 的类型列表方法 `0x462df44` 返回 `71/72`，证明它是撤回相关类型扩展而非通用接收路径。269332+ 的补丁几何仍唯一命中：入口 `0x462e200`，`revoke` 守卫 `entry+0x270`=`0x462e470`（`40100034`→`82000014`），`newmsgid` 写入 `entry+0xA10`=`0x462ec10`（`60CE00F9`→`7FCE00F9`），输出字段仍为 `0x198`/`0x1A0`。通用 Message 终结器 `sub_45CE1A4` 的入口为 `0x45ce1a4`；网络构造函数 `sub_45CCE50` 先填入 server ID `+0xF8`、msgType `+0x0C`、content `+0x130`，再调用终结器，而终结器从 `+0x218` 取扩展对象并通过 vtable `+0x18` 分发，故在此处增加第二个 receive-cache hook。`__DATA` 结束仍为 `0x9aa0000`、`__common` 结束为 `0x9a9ea68`；撤回槽 `0x9a9ff00` 的入口桩为 `90A302B0108247F900021FD6`，相邻接收槽 `0x9a9ff08` 的入口桩为 `90A602B0108647F900021FD6`。屏蔽更新 8 处为 `0x26e4c0`、`0x2706ec`、`0x2709bc`、`0x270ddc`、`0x27b1d0`、`0x27b1d8`、`0x27b1e0`、`0x27b1e8`，原始字节与既有方法语义一致。真实 `/Applications/WeChat.app` 已通过 `install --dry-run` 的 silent / runtime-tip / block-update 三种模式逐点确认。

- **269341**：微信 4.1.12 热修（`CFBundleVersion` 269341，当前 macOS 安装版，2026-07-28 发布）。**整片 arm64 切片被重建而非重定位**：269340 与 269341 两个 thin arm64 切片大小同为 `162178896` 字节，但 `__TEXT` 区段逐字比较 **100% 不一致**，269340 入口 `0x462e200` 在 269341 里已属于另一函数 `_WeChatMain`，旧的几何特征（prologue + `cbz w0,+0x208@+0x270` + `str x0,[x19,#0x198]` + 四处 `ldr x0,[x19,#0x1A0]`）整片零命中。**`__DATA` 布局却逐字节等同 269340**（`vmaddr 0x95ac000`、`vmsize 0x4f4000`、`__common` 结束 `0x9a9ea68`、`__DATA` 结束 `0x9aa0000`），故 SLOT 空隙 `0x9a9ff00`/`0x9a9ff08` 仍有效、消息结构体偏移应未变。按 MAINTAINING「几何特征失配」流程做**参考二进制 diff**：参考取自腾讯 CDN 的 269340 安装包（`dldir1v6.qq.com/weixin/Universal/Mac/xWeChatMac_universal_4.1.12.28_269340.dmg`，`CFBundleVersion` 实测 269340）。取 269340 `parseRevokeXML` 入口 `0x462e200` 起 `0xB00` 字节函数体，屏蔽地址相关立即数（`adrp`/`adr`/`b`/`bl`/`b.cond`/`cbz`/`tbz`/`ldr literal`）后做整片滑窗匹配：**唯一强命中在 `0x462e60c`**（稀疏采样 99%，逐字 95.88%；29 处不匹配全是 `ldr` 向量（`fd4xxxxx`）其 adrp 页基址相对 269340 偏移 `+0x1000`，属栈帧/对象相对偏移，与 GP 寄存器字段访问无关）。逐点核对：入口 prologue 仍是 `F85FBCA9 F65701A9 F44F02A9`；`revoke` 守卫在 `entry+0x270`=`0x462e87c`（`40100034`→`82000014`）；`newmsgid` 写入在 `entry+0xA10`=`0x462f01c`（`60CE00F9`→`7FCE00F9`，即 `str x0,[x19,#0x198]`）；输出字段仍 `0x198`/`0x1A0`。**关键：入口 `0x462e60c` 与 269340 入口 `0x462e200` 同处一个 4KB 页（`0x462e000`）**，故 `adrp/ldr/br` 入口桩与 269340 **逐字节相同**（`90A302B0108247F900021FD6`），`decodeEntryStubSlot` 回环解码到同一 SLOT `0x9a9ff00`。通用 Message 终结器同样被重建并重定位：取 269340 `0x45ce1a4` 起 `0x140` 字节函数体做 masked-shape 匹配，**唯一命中在 `0x45ce5b0`，逐字 100% 一致**，前缀 `ldrb w8,[x0,#0x14a]/cmp/ccmp` 不变（`084049391F0500712008407A`），continuation 在 `entry+0xc`=`0x45ce5bc`；它也与 269340 入口同处 `0x45ce000` 页，故接收槽入口桩 `90A602B0108647F900021FD6` 逐字节不变，回环到 SLOT `0x9a9ff08`；server ID/msgType/content 字段仍 `0xF8`/`0x0C`/`0x130`，故 `{content}` 预览在 269341 同样可用。屏蔽更新：经本构建 `XAppUpdateManager` 的 `__objc_methlist` 相对方法表（91 个方法）按 selector 重新解析，`startUpdater` `0x26e4c0`、`checkForUpdates:` `0x2706ec`、`startBackgroundUpdatesCheck:` `0x2709bc`、`enableAutoUpdate:` `0x270ddc`、访问器对 `0x27b1d0`/`0x27b1d8`/`0x27b1e0`/`0x27b1e8`——**8 个地址与入口原始字节全部与 269340 逐字节一致**（`XAppUpdateManager` 代码区域未被本次重建重定位）。真实 269341 `wechat.dylib`（来自 `Downloads/WeChatMac_4.1.12/WeChat.app`，构建 `269341`）已通过 `install --dry-run` 的 silent / runtime-tip / block-update 三种模式逐点确认。
